...
def insert_random_alarm():
    ...
    try:
        ...
        with sqlite3.connect(DB_PATH) as conn:
            cur = conn.cursor()
            cur.execute(
                "SELECT COUNT(*) FROM alarms WHERE zone_id = ? AND potvrda = 0",
                (zone_id,),
            )
...
tab1, tab2, tab3, tab4 = st.tabs(
    ["🧪 Simulator", "🔴 Aktivni Alarmi", "📋 Pregled Alarma", "📊 Statistike"]
)
...
with tab2:
    ...
    if not df_aktivni.empty:
        ...
        with col1:
            ...
            if st.button("🗑️ Reset Alarm", type="secondary", width="stretch"):
        ...
        with col2:
            ...
            if not osoblje_df.empty:
                ...
                selected_osoblje_ime = osoblje_df[
                    osoblje_df["id"] == selected_osoblje_id
                ]["ime"].iloc[0]
        ...
        if st.button("🗑️ Reset Svih Aktivnih Alarma", type="secondary", width="stretch"):
            try:
                with sqlite3.connect(DB_PATH) as conn:
                    cur = conn.cursor()
...
with tab3:
    ...
    # Get filtered alarms
    alarms_df = get_all_alarms(
        filter_korisnik, filter_osoblje, datum_od_str, datum_do_str
    )
    ...
    with st.expander("🗑️ Brisanje Starih Alarma", expanded=False):
        ...
        try:
            with sqlite3.connect(DB_PATH) as conn:
                cur = conn.cursor()
                cur.execute(
                    "SELECT COUNT(*) FROM alarms WHERE DATE(vrijeme) < ?",
                    (datum_brisi_str,),
                )
...
with tab4:
    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            ...
            with col1:
                cur.execute(
                    "SELECT COUNT(*) FROM alarms WHERE DATE(vrijeme) = DATE('now')"
                )
            ...
            if not weekly_stats.empty:
                ...
                # Chart
                st.line_chart(
                    weekly_stats.set_index("datum")[["potvrđeni", "nepotvrđeni"]]
                )
            else:
...
st.markdown("---")
st.markdown(
    "<sub>💡 Ovaj sustav kombinira simulator alarma s punim upravljanjem alarmnog sustava</sub>",
    unsafe_allow_html=True,
)
