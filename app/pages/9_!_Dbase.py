...
with col3:
    if os.path.exists(DB_PATH):
        try:
            with sqlite3.connect(DB_PATH) as conn:
                total_records = sum(get_table_stats(conn, table) for table in get_table_names(conn))
...
tab1, tab2, tab3, tab4 = st.tabs(["🔧 Provjera & Inicijalizacija", "📋 Pregled Tablica", "🛠️ Održavanje", "📊 Analiza"])
...
with tab1:
            ...
            with sqlite3.connect(DB_PATH) as conn:
                ...
                for table_name, columns in REQUIRED_TABLES.items():
                    if table_has_columns(conn, table_name, columns):
                        ...
                        st.warning(f"⚠️ **{table_name}** - nedostaju kolone ili tablica ne postoji")
                        all_ok = False
    ...
    with col1:
        if st.button("🔄 Inicijaliziraj/Popravi Bazu", use_container_width=True):
            with st.spinner("Inicijaliziram bazu..."):
                missing_tables = init_database()
                if missing_tables:
                    st.success(f"✅ Tablice stvorene/ažurirane: {', '.join(missing_tables)}")
                else:
                    st.info("ℹ️ Sve tablice već postoje i ispravne su")
    
    with col2:
        if st.button("💾 Stvori Backup", use_container_width=True):
                ...
                if not tables:
                    ...
                    if selected_table:
                        ...
                        if df.empty:
                            ...
                            with col2:
                                show_all = st.checkbox("Prikaži sve zapise", value=len(df) <= 100)
                            
                            if show_all or len(df) <= 100:
                                st.dataframe(df, use_container_width=True, height=400)
                            else:
                                st.dataframe(df.head(100), use_container_width=True, height=400)
                                st.info(f"Prikazano prvih 100 od {len(df)} zapisa. Označite 'Prikaži sve zapise' za prikaz svih.")
            ...
            with sqlite3.connect(DB_PATH) as conn:
                ...
                if tables:
                    ...
                    if selected_table_clear:
                        ...
                        st.warning(f"Tablica `{selected_table_clear}` ima {record_count} zapisa")
                        
                        if st.button(f"🗑️ Obriši sve podatke iz `{selected_table_clear}`", type="secondary"):
                            if clear_table_data(selected_table_clear):
                                st.success(f"✅ Svi podaci iz tablice `{selected_table_clear}` su obrisani")
                                st.rerun()
                    
                    st.markdown("#### 📊 Optimizacija")
                    if st.button("🔧 Optimiziraj bazu podataka", use_container_width=True):
                        try:
                            with sqlite3.connect(DB_PATH) as conn:
                                conn.execute("VACUUM")
                                conn.commit()
        ...
        try:
            with sqlite3.connect(DB_PATH) as conn:
                ...
                if tables:
                    ...
                    with col2:
                        if not df_analysis.empty and df_analysis["Broj zapisa"].sum() > 0:
                            st.bar_chart(df_analysis.set_index("Tablica")["Broj zapisa"])
                    ...
                    if "zone" in tables and "korisnici" in tables:
                        ...
                        if not zone_df.empty:
                            ...
                            with col1:
                                ...
                                assigned_zones = len(zone_df[zone_df["korisnik_id"].notna()])
                            ...
                            with col2:
                                ...
                                if total_zones > 0:
                                    ...
                                    st.metric("Postotak korištenja", f"{usage_percent:.1f}%")
                else:
